...
function prepareTransactions(lines, startBalance) {
  ...
  var grouped = groupBy_1(records, function (d) { return d.date.getTime();});

  var groupedRecords = values_1(grouped).map(function (g) {
    return {
      date: g[0].date,
      amount: sum_1(g.map(function (d) { return d.amount; })),
      transactions: g
    };
  });
